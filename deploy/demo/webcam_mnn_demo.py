    ...
    with torch.no_grad():
        while cap.isOpened():
            ...
            if location is not None:
                ...
                face_roi = cv2.resize(face_roi, (36, 36))
                if frame_count > START_FRAME:
...
def inference(face_queue, method='dft'):
    global hr_predict, input_tensor, interpreter, session
    ...
    cur_faces = to_onnxruntime_input(cur_faces)
    cur_faces = cur_faces.astype(np.float32)

    tmp_input = MNN.Tensor((1, 3, 240, 36, 36), MNN.Halide_Type_Float,
                           cur_faces, MNN.Tensor_DimensionType_Caffe)
    ret = input_tensor.copyFrom(tmp_input)
    ret = interpreter.runSession(session)
    output_tensor = interpreter.getSessionOutput(session)
    # constuct a tmp tensor and copy/convert in case output_tensor is nc4hw4
    tmp_output = MNN.Tensor((1, 240), MNN.Halide_Type_Float, np.ones([1, 240]).astype(np.float32),
                            MNN.Tensor_DimensionType_Caffe_C4)
    output_tensor.copyToHostTensor(tmp_output)
    output_tensor_np = output_tensor.getNumpyData()

    hr_predict = postprocess(output_tensor_np[0, ], fps=DETECT_FPS_COUNT, length=FRAME_LEN, method=method)
...
def main():
    global interpreter, session, hr_predict, face_tracker, location, input_tensor
    hr_predict = None
    location = None

    # 0. set path
    video_path = '0'
    rppg_model_path = '../model/EfficientPhys_Conv.mnn'

    # 1. build tracker
    face_tracker = cv2.TrackerKCF_create()
    # 2. build rPPG net
    interpreter = MNN.Interpreter(rppg_model_path)
...
if __name__ == '__main__':
    main()
    # 这里如果使用(1, 3, 240, 128, 128)会出现Can't run session because not resized。具体原因见https://github.com/alibaba/MNN/issues/2204，解决方法是减小输入尺寸
