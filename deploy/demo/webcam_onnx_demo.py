...
def inference(face_queue, method='dft'):
    ...
    inputs = {input_name: cur_faces}
    output_wave = rppg_net.run(None, inputs)
    wave_predict = output_wave[0][0, :]
...
def main():
    global rppg_net, hr_predict, face_tracker, location, input_name, outpu_name
    hr_predict = None
    location = None

    # 0. set path
    video_path = '0'
    rppg_model_path = '../model/EfficientPhys_Conv.sim.onnx'
    ...
    print(providers)
    rppg_net = onnxruntime.InferenceSession(rppg_model_path, providers=providers)
    input_name = rppg_net.get_inputs()[0].name
    outpu_name = rppg_net.get_outputs()[0].name
